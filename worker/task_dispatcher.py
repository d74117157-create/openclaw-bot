"""
OpenClaw Task Dispatcher — routes tasks from all platforms → Agents → Results → Storage
FIXED: Sync agents run in thread pool to prevent event loop blocking.
"""
import asyncio
import logging
...
class TaskDispatcher:
    """Central task router — receives from all platforms, dispatches to agents."""
    ...
    async def execute_task(self, task_id: str) -> str:
        """Execute a pending task via its assigned agent (in thread pool to avoid blocking)."""
        ...
        try:
            ...
            # FIXED: Run sync agent in thread pool to avoid blocking event loop
            loop = asyncio.get_running_loop()
            result = await loop.run_in_executor(None, agent_fn, task_desc)
