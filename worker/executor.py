"""
OpenClaw — worker/executor.py
Parallel execution engine. Runs agents concurrently, posts Slack + GitHub.
"""
...
class ExecutorEngine:
    ...
    def run_browser(self, browser_task: str, parent_task: str = "") -> str:
        try:
            ...
            return (
                ...
                f"Steps done: {result.get('steps_done',0)} | "
    ...
    def self_test(self, task: str, agent_results: dict) -> dict:
        results_text = "\n\n".join(
            f"[{agent.upper()}]:\n{str(out)[:500]}"
        ...
        qa_prompt = (
            ...
            f"Agent outputs:\n{results_text}\n\n"
            f"Run a QA check. Evaluate:\n"
            f"1. Did agents address the task fully?\n"
            f"2. Are there errors or gaps?\n"
            f"3. Is output production-ready?\n"
            f"4. Any security issues?\n\n"
            f"Return JSON:\n"
            f'{{"passed": true, "summary": "...", "issues": [], '
            f'"tests_run": 1, "tests_passed": 1, "recommendations": []}}'
        )
        try:
        ...
        try:
            ...
            slack.chat_postMessage(
                ...
                blocks=[
                    ...
                    {"type": "section",
                     "text": {"type": "mrkdwn", "text": f"*Task:* {task[:150]}"}},
                    {"type": "section",
                     "text": {"type": "mrkdwn", "text": f"*QA:* {verdict}"}},
                    {"type": "section",
                     "text": {"type": "mrkdwn", "text": f"```{preview}```"}},
                    {"type": "divider"},
                    {"type": "context",
                     "elements": [{"type": "mrkdwn",
                                   "text": f"{time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())}"}]},
                ]
            )
        except Exception as e:
            print(f"[Executor] Slack post failed: {e}")

    def post_slack_agent_update(self, agent: str, task: str, result: str, status: str = "OK"):
        if not SLACK_BOT_TOKEN:
            return
        try:
            from slack_sdk import WebClient
            ...
            slack.chat_postMessage(
                ...
                blocks=[
                    {"type": "section",
                     "text": {"type": "mrkdwn",
                              "text": f"{status} *[{agent.upper()}]* `{task[:80]}`"}},
                    {"type": "section",
                     "text": {"type": "mrkdwn", "text": f"```{preview}```"}},
                ]
            )
        except Exception as e:
            print(f"[Executor] Slack agent update failed: {e}")
    ...
    def post_github_issue(self, task: str, summary: str) -> str:
        ...
        try:
            ...
            body = (
                ...
                f"- Source: OpenClaw Discord BrainBot\n"
            )
            resp = requests.post(
                f"{GITHUB_API}/repos/{GITHUB_REPO}/issues",
                headers=headers,
                json={
                    "title": f"[OpenClaw] {task[:80]}",
                    "body":  body,
                    "labels": ["openclaw", "swarm-task"],
                },
                timeout=15,
            )
    ...
    def create_github_pr(self, title: str, body: str, head: str, base: str = "main") -> str:
        if not GITHUB_TOKEN or not GITHUB_REPO:
            return "GitHub not configured"
        try:
            import requests
            ...
            resp.raise_for_status()
            data = resp.json()
            return f"PR #{data['number']}: {data['html_url']}"
        except Exception as e:
            return f"GitHub PR failed: {e}"
