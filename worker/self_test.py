"""QA self-test harness. Validates swarm outputs."""
import os
import json
import time
import logging
...
logger = logging.getLogger("openclaw.self_test")
...
def run_self_test(task: str, results: dict) -> dict:
    ...
    prompt = (
        ...
        f"AGENT OUTPUTS:\n{results_block}\n\n"
        f"Run comprehensive QA validation. Check: 1) Task completion 2) Output quality 3) Errors 4) Security 5) Completeness. Return JSON only."
    )
    try:
        ...
        logger.warning(f"AI QA failed: {e}")
