...
echo "[OpenClaw Elite] Discord + Telegram + Slack + All Agents"
echo "[OpenClaw Elite] ============================================"

# Only require DISCORD_TOKEN (or any gateway token)
if [ -z "${DISCORD_TOKEN:-}" ] && [ -z "${TELEGRAM_BOT1_TOKEN:-}" ] && [ -z "${SLACK_BOT_TOKEN:-}" ]; then
    ...
    echo "[OpenClaw Elite]   - TELEGRAM_BOT1_TOKEN (for Telegram)"
...
echo "[OpenClaw Elite] Active gateways: Discord, Telegram, Slack"
