...
async def _update_live_embed(msg, task, statuses, plan):
    ...
    e.add_field(name="Agent Progress", value=lines or "Starting...", inline=False)
...
class BrainBot(discord.Client):
    ...
    async def _handle_task(self, message: discord.Message, task: str):
        ...
        plan_embed.add_field(
            name=f"Execution Plan ({len(valid_plan)} agents)",
            value=agent_lines or "orchestrator",
            inline=False,
        )
        if needs_browser:
            plan_embed.add_field(name="\U0001f310 Web", value="Browser agent active", inline=True)
        ...
        qa_result  = await asyncio.get_event_loop().run_in_executor(
            None, lambda: self.executor.self_test(task, results)
        )
        qa_verdict = "PASSED" if qa_result.get("passed") else "FAILED"
        qa_embed   = discord.Embed(
            title=f"\U0001f9ea QA Self-Test \u2014 {qa_verdict}",
            description=qa_result.get("summary", ""),
            color=CLR_TEST,
        )
        ...
        final_embed = discord.Embed(
            title="\U0001f3c1 Task Complete \u2014 OpenClaw Swarm OS",
            description=f"**Task:** {task[:200]}",
            color=CLR_DONE,
        )
        final_embed.add_field(name="Agents Run", value=str(len(valid_plan)), inline=True)
        final_embed.add_field(name="QA Status",  value=qa_verdict,          inline=True)
        final_embed.set_footer(text=f"OpenClaw | {_ts()}")
        await channel.send(embed=final_embed)
        summary_text = "\n\n".join(
            f"**{p['agent'].upper()}:**\n{results.get(p['agent'],'')[:300]}"
            for p in valid_plan
        )
        asyncio.get_event_loop().run_in_executor(
            None, lambda: self.executor.post_slack(task, summary_text, qa_result)
        )
        asyncio.get_event_loop().run_in_executor(
            None, lambda: self.executor.post_github_issue(task, summary_text)
        )
...
async def agents_cmd(itx: discord.Interaction):
    ...
    for name, desc in AGENT_PERSONAS.items():
        e.add_field(
            name=f"{AGENT_EMOJI.get(name,'\U0001f916')} {name.upper()}",
            value=desc[:100],
            inline=False,
        )
...
async def stop_cmd(itx: discord.Interaction):
    e = discord.Embed(
        title="\U0001f6d1 STOP ACKNOWLEDGED",
        description="All running operations flagged.\nRailway: `railway down`",
        color=0xff0000,
    )
    await itx.response.send_message(embed=e)
        ...
        if not content:
            await message.channel.send(
                embed=discord.Embed(
                    ...
                    description="Tell me **anything** \u2014 I'll decompose it into agent tasks and execute hands-free.",
                    color=CLR_DONE,
